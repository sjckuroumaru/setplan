...
describe('Issues Page - Filters', () => {
  ...
  it('should filter issues by search query', () => {
    ...
    // 検索クエリを入力
    cy.get('#search').type('ログイン');

    // 検索結果が表示される（フィルターは自動的に適用される）
  ...
  it('should filter issues by status', () => {
    ...
    cy.contains('[data-slot="select-item"]', '対応中', { timeout: 10000 }).should('be.visible').click();
  ...
  it('should filter issues by priority', () => {
    ...
    // 優先度フィルターで「高」を選択
    cy.get('#priority-filter').click();
    cy.contains('[data-slot="select-item"]', '高', { timeout: 10000 }).should('be.visible').click();
  ...
  it('should filter issues by project', () => {
    ...
    // 案件フィルターを選択（最初の案件）
    cy.get('#project-filter').click();
    cy.get('[data-slot="select-item"]').eq(1).click();
  ...
  it('should filter issues by department', () => {
    cy.visit('/issues');
