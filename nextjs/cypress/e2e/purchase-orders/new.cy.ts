describe('発注書新規作成ページ', () => {
  ...
  describe('基本機能', () => {
    it('should display the new purchase order form', () => {
      cy.contains('発注書作成').should('be.visible')
  ...
  describe('フォームバリデーション', () => {
    ...
    it('should show error when line item name is empty', () => {
      ...
      cy.get('input[name="subject"]').type('テスト発注書')
  ...
  describe('追加情報入力', () => {
    ...
    it('should fill delivery location field', () => {
      cy.get('input[name="deliveryLocation"]').type('貴社指定場所')
      cy.get('input[name="deliveryLocation"]').should('have.value', '貴社指定場所')
    })
  ...
  describe('フォーム送信', () => {
    it('should successfully create a new purchase order', () => {
      ...
      cy.get('input[name="subject"]').type('新規発注書テスト')
      ...
      // 備考を入力
      cy.get('textarea[name="remarks"]').clear().type('これはテスト用の発注書です。')
