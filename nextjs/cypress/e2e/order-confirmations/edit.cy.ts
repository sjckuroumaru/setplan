describe('発注請書編集ページ', () => {
  let orderConfirmationId: string
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/order-confirmations/*').as('getOrderConfirmation')
    cy.contains('[role="menuitem"]', '編集').should('be.visible').click()

    // URLが編集ページになることを確認
    cy.url().should('include', '/edit')
    cy.url().then((url) => {
      const matches = url.match(/\/order-confirmations\/([^/]+)\/edit/)
      if (matches) {
        orderConfirmationId = matches[1]
      }
    })

    // APIレスポンスを待ち、データ取得が成功したことを確認
    cy.wait('@getOrderConfirmation').then((interception) => {
      // 200 OKでない場合はテストを失敗させる
      expect(interception.response?.statusCode).to.eq(200)
    })

    // 編集フォームが表示されることを確認
    cy.contains('発注請書編集', { timeout: 10000 }).should('be.visible')
  })

  describe('基本機能', () => {
    it('should display the edit purchase order form', () => {
      cy.contains('発注請書編集').should('be.visible')
  ...
  describe('データ更新', () => {
    ...
    it('should update purchase order subject', () => {
      // 件名を変更
      cy.get('input[name="subject"]').scrollIntoView().should('be.visible').clear().type('更新された発注請書タイトル')
  ...
  describe('ページ遷移', () => {
    ...
    it('should stay on edit page when validation fails', () => {
      ...
      // 見出しが表示されることを確認
      cy.contains('発注請書編集').scrollIntoView().should('be.visible')
    })
  })
