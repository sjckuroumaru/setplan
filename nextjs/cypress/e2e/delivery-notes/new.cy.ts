describe('納品書新規作成ページ', () => {
  ...
  describe('基本機能', () => {
    it('should display the new delivery note form', () => {
      cy.contains('納品書作成').should('be.visible')
    ...
    it('should navigate back to delivery notes list when clicking cancel', () => {
      cy.contains('button', 'キャンセル').click()
      cy.url().should('eq', Cypress.config().baseUrl + '/delivery-notes')
    })
  })
  ...
  describe('フォームバリデーション', () => {
    ...
    it('should show error when line item name is empty', () => {
      ...
      cy.get('input[name="subject"]').scrollIntoView().type('テスト納品書')
  ...
  describe('明細管理', () => {
    ...
    // Note: delivery-notesの実装では最後の1行でも削除可能（disabled属性なし）
  ...
  describe('フォーム送信', () => {
    it('should successfully create a new delivery note', () => {
      // 必須フィールドを入力
      cy.contains('label', '顧客').scrollIntoView().parent().find('button[role="combobox"]').click()
      cy.get('[role="option"]').first().click()

      cy.get('input[name="subject"]').scrollIntoView().type('新規納品書テスト')
      ...
      // 備考を入力
      cy.get('textarea[name="remarks"]').scrollIntoView().clear().type('これはテスト用の納品書です。')
    ...
    it('should create delivery note with mixed tax rates', () => {
      // 必須フィールドを入力
      cy.contains('label', '顧客').scrollIntoView().parent().find('button[role="combobox"]').click()
      cy.get('[role="option"]').first().click()
