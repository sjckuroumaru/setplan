describe('納品書編集ページ', () => {
  ...
  beforeEach(() => {
    ...
    // 編集対象の納品書IDを取得（下書きステータスの納品書のみ編集可能）
    ...
    cy.intercept('GET', '/api/delivery-notes/*').as('getDeliveryNote')
    cy.contains('[role="menuitem"]', '編集').should('be.visible').click()

    // URLが編集ページになることを確認
    cy.url().should('include', '/edit')
    cy.url().then((url) => {
      const matches = url.match(/\/delivery-notes\/([^/]+)\/edit/)
      if (matches) {
        deliveryNoteId = matches[1]
      }
    })

    // APIレスポンスを待ち、データ取得が成功したことを確認
    cy.wait('@getDeliveryNote').then((interception) => {
      // 200 OKでない場合はテストを失敗させる
      expect(interception.response?.statusCode).to.eq(200)
    })

    // 編集フォームが表示されることを確認
    cy.contains('納品書編集', { timeout: 10000 }).should('be.visible')
  })

  describe('基本機能', () => {
    it('should display the edit delivery note form', () => {
      cy.contains('納品書編集').should('be.visible')
      cy.get('form').should('be.visible')
    })

    it('should load existing delivery note data', () => {
      // フォームが読み込まれるまで待機
      cy.contains('納品書編集').should('be.visible')
    ...
    it('should display all form fields', () => {
      ...
      cy.get('input[name="deliveryDate"]').scrollIntoView().should('be.visible')
      cy.contains('label', '顧客').scrollIntoView().should('be.visible')
      cy.contains('label', '敬称').scrollIntoView().should('be.visible')
    })

    it('should navigate back to delivery note detail when clicking cancel', () => {
  ...
  describe('データ更新', () => {
    ...
    it('should update delivery note subject', () => {
      // 件名を変更
      cy.get('input[name="subject"]').scrollIntoView().should('be.visible').clear().type('更新された納品書タイトル')
  ...
  describe('明細更新', () => {
    ...
    it('should add new line item to existing delivery note', () => {
      // 新しい明細を追加
      cy.contains('button', '明細追加').scrollIntoView().click()
    ...
    it('should remove line item from delivery note', () => {
      // まず明細を追加（削除テストのため）
      cy.contains('button', '明細追加').scrollIntoView().click()
  ...
  describe('ページ遷移', () => {
    ...
    it('should navigate back to delivery notes list after successful update', () => {
      // 件名を変更
    ...
    it('should stay on edit page when validation fails', () => {
      ...
      // 見出しが表示されることを確認
      cy.contains('納品書編集').scrollIntoView().should('be.visible')
    })
  })
