...
export function OrderConfirmationForm({ initialData, isEditMode = false }: OrderConfirmationFormProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [customers, setCustomers] = useState<Customer[]>([])

  const form = useForm<OrderConfirmationFormData>({
    resolver: zodResolver(OrderConfirmationFormSchema) as any,
  ...
  const onSubmit = async (data: OrderConfirmationFormData) => {
    setLoading(true)

    try {
      // 空の明細行を除外
      const filteredItems = data.items.filter(item => item.name && item.name.trim() !== '')
      ...
      const url = isEditMode && initialData 
        ? `/api/order-confirmations/${initialData.id}` 
      ...
      router.push(`/order-confirmations/${result.orderConfirmation.id}`)
    } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-2">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push("/order-confirmations")}
          >
            キャンセル
          </Button>
          <Button
            ...
            onClick={() => {
              ...
              const filteredItems = values.items.filter(item => item.name && item.name.trim() !== '')
