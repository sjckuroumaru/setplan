...
export async function PUT(
  ...
  try {
    ...
    // 既存の発注請書を確認
    const existingOrderConfirmation = await prisma.orderConfirmation.findUnique({
      where: { id: resolvedParams.id },
    })

    if (!existingOrderConfirmation) {
      return NextResponse.json({ error: "発注請書が見つかりません" }, { status: 404 })
    }

    // 権限チェック（管理者または作成者のみ）
    if (!session.user.isAdmin && existingOrderConfirmation.userId !== session.user.id) {
    ...
    console.error("Order confirmation status update error:", error)
