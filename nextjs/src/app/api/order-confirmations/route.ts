...
export async function GET(request: NextRequest) {
  try {
    ...
    const [orderConfirmations, total] = await Promise.all([
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Order confirmation create error:", error)
    return NextResponse.json({ error: "発注請書の作成に失敗しました" }, { status: 500 })
  }
}
